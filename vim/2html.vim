...
if exists('g:loaded_diffchar')
  let s:matches = {}
  let pr = (g:loaded_diffchar >= 6.5) ? '^-[23]$' : '^0$'
  let gm = filter(getmatches(), 'v:val.priority =~ pr')
  for m in gm
    for [p, l] in filter(items(m), 'v:val[0] =~ "^pos\\d\\+$"')
      if len(l) == 1
	let s:matches[l[0]] = [m.group]
      endif
    endfor
  endfor
  for m in gm
    for [p, l] in filter(items(m), 'v:val[0] =~ "^pos\\d\\+$"')
      if len(l) == 3
	let s:matches[l[0]] += [[[l[1], l[1] + l[2] - 1], m.group]]
      endif
    endfor
  endfor
  function! s:Diff_hlID(l, c)
    if has_key(s:matches, a:l)
      for [se, gr] in s:matches[a:l][1:]
	if se[0] <= a:c && a:c <= se[1]
	  let id = hlID(gr)
	  if !has_key(s:diffstylelist, id)
	    let s:diffstylelist[id] = '.' . gr . ' { ' . s:CSS1(id) . '}'
	  endif
	  return id
	endif
      endfor
      return hlID(s:matches[a:l][0])
    endif
    return diff_hlID(a:l, a:c)
  endfunction
else
  function! s:Diff_hlID(l, c)
    return diff_hlID(a:l, a:c)
...
while s:lnum <= s:end
  ...
  if has('folding') && !s:settings.ignore_folding && foldclosed(s:lnum) > -1 && !s:settings.dynamic_folds
    ...
    let s:diffattr = s:Diff_hlID(s:lnum, 1)
	...
	while s:col <= s:len && s:concealinfo == synconcealed(s:lnum, s:col) | let s:col = s:col + 1 | endwhile
      elseif s:diffattr
	let s:diff_id = s:Diff_hlID(s:lnum, s:col)
	let s:id = synID(s:lnum, s:col, 1)
	let s:col = s:col + 1
	" Speed loop (it's small - that's the trick)
	" Go along till we find a change in hlID
	while s:col <= s:len && s:id == synID(s:lnum, s:col, 1)
	      \   && s:diff_id == s:Diff_hlID(s:lnum, s:col) |
	      \     let s:col = s:col + 1 |
	      \ endwhile
