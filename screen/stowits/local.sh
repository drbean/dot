...
declare -a cmd=( "active_tooter" "blog.sh" "common.sh" "easyblogger" "get_story_out.sh" "masto_master_messter" "post.sh" "pub" "toot" "toot_instance" "trurl" "webcaster.sh")
echo /usr/local/bin command="( ${cmd[@]} )"

declare -A key_cmd=(
    [mastodon]="{active_tooter,masto_master_messter,toot_instance,webcaster.sh}"
